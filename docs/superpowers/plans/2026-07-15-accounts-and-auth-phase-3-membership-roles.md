...
- **Addendum — admin team creation (brought forward from Phase 7):** migration `0006_team_write_grants` grants `insert/update/delete on team` to `authenticated` (RLS "admin manages teams" already restricts rows to the admin). Added `createTeam` server action + an admin-only `CreateTeamForm` at the top of `/team`; a new team appears as an empty panel with its own add-by-email form (admins see all teams via `is_admin`). The page no longer 404s for an admin with zero teams.
