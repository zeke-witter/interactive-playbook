...
- **`NarrativePanel.tsx`** — shows the current step's narrative for the selected position (or a "you're off the disc" fallback). Delegates term/tooltip rendering to `NarrativeWithTooltips`. Includes a **dev-only** inline editor (`NODE_ENV === 'development'`) that PATCHes `/api/plays/[playId]/narrative` then reloads.
